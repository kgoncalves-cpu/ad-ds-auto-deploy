...
Write-Host "   Windows Server 2022 - Versão 1.0" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""

Write-Log "Iniciando script de implementação do Domain Controller" -Level Info
...
do {
    ...
    Write-Log "Segmento $segmentCount adicionado: $networkIP/$subnetMask (Gateway: $gateway)" -Level Success
...
try {
    ...
    $gateway = $config.NetworkSegments[0].Gateway
    ...
    Write-Log "IP estático configurado: $($config.ServerIP)" -Level Success
} catch {
...
if ($needsReboot) {
    ...
    $config | Export-Clixml -Path "C:\ADConfig_Continue.xml"
    ...
    if ($rebootNow -eq 'S' -or $rebootNow -eq 's') {
        ...
        Write-Log "Reinicie o servidor manualmente e execute o script novamente" -Level Warning
        exit
    }
}
...
$config | Export-Clixml -Path "C:\ADConfig_Part2.xml"
...
if (-not (Test-Path "C:\ADConfig_Part2.xml")) {
    Write-Host "Arquivo de configuração não encontrado!" -ForegroundColor Red
    exit
}

$config = Import-Clixml -Path "C:\ADConfig_Part2.xml"
...
$part2Script | Out-File -FilePath "C:\ADConfig_Part2.ps1" -Encoding UTF8

Write-Host "`n✅ Script da Parte 2 criado: C:\ADConfig_Part2.ps1" -ForegroundColor Green
Write-Host "`nApós reinicialização, execute: PowerShell.exe -ExecutionPolicy Bypass -File C:\ADConfig_Part2.ps1" -ForegroundColor Yellow